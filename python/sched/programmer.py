...
class Programmer:
    ...
    def run1(self, now=None):
        ...
        while len(queue) > 0 and queue[0][0] <= now:
            ...
            logging.debug(
                "programmer starting instance: {}".format(instance.id))
...
def program_instance(queue, instance, time):
    assert _is_datetime(time)

    heapq.heappush(queue, (time, instance))


def program_instance_in(queue, instance, seconds):
    program_instance(queue, instance, _future(seconds))
