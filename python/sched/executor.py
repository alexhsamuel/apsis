...
class Executor:
    ...
    def __init__(self, execute_queue, instance_db):
        self.__execute_queue = execute_queue
    ...
    def start(self):
        ...
        """
        while len(self.__execute_queue) > 0:
            instance = self.__execute_queue.pop()
            logging.debug("executor starting instance {}".format(instance.id))
    ...
    def run_all(self, interval=0.1):
        while len(self.__execute_queue) > 0 or len(self.__procs) > 0:
            self.run1()
            # FIXME: Hacky.
            time.sleep(interval)
...
def execute_instance(execute_queue, instance):
    """
    Enqueues an instance for immediate execution.
    """
    execute_queue.append(instance)
